...
void main() {
  ...
  // but DON'T mix multiple colors.

  for (int i = 0; i < MAX_COLORS; ++i) {

    if (i >= max(uColorCount, 1)) break;
...
export default function ColorBends({
  ...
  const rotationRef =
    useRef<number>(rotation);

  const autoRotateRef =
    useRef<number>(autoRotate);

  const pointerTargetRef =
    useRef<THREE.Vector2>(
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      (
        material.uniforms
          .uCanvas.value as THREE.Vector2
      ).set(
        w,
        h
      );
